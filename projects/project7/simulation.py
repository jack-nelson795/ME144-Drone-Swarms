...
class Simulation:
    ...
    def __init__(self, parameters, LAM):
        ...
        self.GX: float = float(parameters["GX"])
        self.GY: float = float(parameters["GY"])
        self.GZ: float = float(parameters["GZ"])
        self.DT: float = float(parameters["DT"])
        self.STEPS: int = int(parameters["STEPS"])
        self.FRACTION_PARTICLES_TRACKED: float = float(parameters["FRACTION_PARTICLES_TRACKED"])
        ...
        self.FIRE_ZONE_SHAPE: str = str(parameters["FIRE_ZONE_SHAPE"])
        self.FIRE_CENTER_X: float = float(parameters["FIRE_CENTER_X"])
        self.FIRE_CENTER_Z: float = float(parameters["FIRE_CENTER_Z"])
        self.FIRE_RADIUS: float = float(parameters["FIRE_RADIUS"])
        self.UPDRAFT_VELOCITY: float = float(parameters["UPDRAFT_VELOCITY"])
        self.UPDRAFT_DECAY: float = float(parameters["UPDRAFT_DECAY"])
        self.NOZZLE_AFT_OFFSET: float = float(parameters["NOZZLE_AFT_OFFSET"])
        self.NOZZLE_DROP_OFFSET: float = float(parameters["NOZZLE_DROP_OFFSET"])
        self.TANK_VOLUME: float = float(parameters["TANK_VOLUME"])
        self.RETARDANT_DENSITY: float = float(parameters["RETARDANT_DENSITY"])
        self.Y0_MAX: float = float(parameters["Y0_MAX"])
        self.Y0_MIN: float = float(parameters["Y0_MIN"])
        self.W_1: float = float(parameters["W_1"])
        self.W_2: float = float(parameters["W_2"])
        self.WIND_X: float = float(parameters["WIND_X"])
        self.WIND_Y: float = float(parameters["WIND_Y"])
        self.WIND_Z: float = float(parameters["WIND_Z"])
        self.MU_F: float = float(parameters["MU_F"])
        self.RHO_A: float = float(parameters["RHO_A"])
        self.SPACING: float = float(parameters["SPACING"])
    ...
    def _update_aircraft_state(self, airplane, curve):
        pos     = curve.get_position(self.u)
        tangent = curve.get_tangent(self.u)

        tangent_norm = np.linalg.norm(tangent)
        du       = airplane.speed * self.DT / tangent_norm
        self.u  += du
    ...
    def _compute_nozzle_positions(self, airplane, plane_right, count):
        ...
        base_positions[nozzle_idx == 1] += airplane.spacing * plane_right
        base_positions[nozzle_idx == 2] -= airplane.spacing * plane_right
        base_positions[nozzle_idx == 3] -= airplane.spacing * airplane.dir

        end_positions = base_positions - self.NOZZLE_DROP_OFFSET * plane_up
    ...
    def _compute_spray_velocities(self, airplane, plane_right, sprayer_base, sprayer_end, count):
        ...
        directions = n_base + self.spray_amplitude * n_rand
        ...
        return v_nozzle + self.eject_speed * n_spray
    ...
    def _release_droplets(self, airplane, plane_right, t):
        ...
        positions = sprayer_end + self.DT * velocities
    ...
    def _compute_updraft(self, r_curr):
        ...
        if np.any(in_fire):
            ...
            v_up[in_fire, 1]  = self.UPDRAFT_VELOCITY * np.exp(-self.UPDRAFT_DECAY * np.maximum(0, y_heights))
    ...
    def _compute_drag(self, v_rel):
        ...
        Re = 2 * self.r_particle * self.RHO_A * safe_speed / self.MU_F
    ...
    def _step_physics(self, flying_mask):
        ...
        acc = f_drag / (self.RETARDANT_DENSITY * self.vol_particle) + self.G

        # Forward Euler integration
        v_curr += acc * self.DT
        r_curr += v_curr * self.DT
    ...
    def simulate_path_with_nozzles(self, seed=144):
        ...
        self.landed[:] = False
        self.drops_released = 0
        self.airplane.pos = np.array([self.airplane.x0, self.airplane.y0, self.airplane.z0], dtype=float)
        self.airplane.dir = np.array([1.0, 0.0, 0.0])
        self.airplane.vel = self.airplane.speed * self.airplane.dir
        ...
        for t in range(self.STEPS):
            ...
            self._step_physics(self.active & ~self.landed)
    ...
    def get_cost_breakdown(self):
        ...
        N_hit     = int(np.sum(in_fire))
        N_total   = int(self.num_particles)
        hit_fraction = N_hit / N_total if N_total > 0 else 0.0
        aircraft_height_term = 1 - self.airplane.y0 / self.Y0_MAX
        cost = self.W_1 * (1 - hit_fraction) + self.W_2 * aircraft_height_term

        return {
            'cost': float(cost),
            'hit_count': N_hit,
            'hit_fraction': float(hit_fraction),
            'num_particles_released': int(self.drops_released),
            'num_particles_total': N_total,
            'aircraft_height_term': float(aircraft_height_term),
            'landed_particles': int(np.sum(self.landed)),
        }

    def calculate_cost(self):
        return self.get_cost_breakdown()['cost']
...
if __name__ == "__main__":
    ...
    fixedLam = True
    if fixedLam:
        LAM = [ 6.93e1,  0.0 , 1.751e2 ,-1.192e1, 4.29e-3,  1.83e-1 , 2.159e-2 , 6.921e-1, 8.21,  4.878e1 , 4.85e+1 , 1.5207e3]
        title = 'fixedLam'
    else:
