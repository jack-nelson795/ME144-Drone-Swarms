...
class GeneticAlgorithm(_GeneticAlgorithmBase):
    ...
    def _generate_design_string(self) -> np.ndarray:
        LAM = np.array([
            self.rng.uniform(50, 100), # aircraft speed in m/s
            0., # x0 fixed at zero so that the plane starts at one end of the domain and not in the middle, give z the freedom
            self.rng.uniform(self.Y0_MIN, self.Y0_MAX), # y0 ranges from 50 to 250 meters above ground
            self.rng.uniform(-100, 100), # z0 ranges from -100 to 100 m
            self.rng.uniform(0.001, 0.01), # particles are coarse for simulation approximation, parallelization would help
            self.rng.uniform(0, 0.5), # controls how wide the "cone" of particles is when they are released from the plane
            self.rng.uniform(0, 0.1) , # we can start spraying anywhere from the beginning to 1/4 of the way into the flight
            self.rng.uniform(0.25, 1) , # we can stop spraying anywhere from 1/4 to the end of the flight 
            self.rng.uniform(2, 10), # volume of retardant dropped per second
            self.rng.uniform(0, 150), # particle drop velocity 
            self.rng.uniform(1, 100), # amplitude of parametric curve
            self.rng.uniform(500, 2000) # wavelength of parametric curve
        ])
        return LAM
